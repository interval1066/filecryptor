...
uint8_t*
AES::generateKeySchedule(uint8_t* keyBytes)
{
...
uint8_t*
AES::aesG(uint8_t* word, int round)
{
...
void
AES::addWords(uint8_t * a, uint8_t * b)
{
    for (int byte = 0; byte < 4; ++byte)
    {
        a[byte] ^= b[byte];
...
void
AES::addKey(uint8_t * bytes, uint8_t * key)
{
...
void
AES::leftCyclicByteShiftWord(uint8_t * word)
{
...
void
AES::byteSubstitution(uint8_t* bytes)
{
    for (int byte = 0; byte < 16; ++byte)
    {
        bytes[byte] = sbox[bytes[byte]];
...
void
AES::inverseByteSubstitution(uint8_t* bytes)
{
    for (int byte = 0; byte < 16; ++byte)
...
void
AES::inverseShiftRows(uint8_t* bytes)
{
    uint8_t mappedBytes[16];
    for (int row = 0; row < 4; ++row) {
        for (int column = 0; column < 4; ++column)
...
void
AES::inverseMixColumns(uint8_t* bytes)
{
    uint8_t tmp[16];
    for(int i = 0; i <= 12; i += 4) {
