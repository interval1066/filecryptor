...
void
AES256_Base::expand_enc_key(Byte_Block<32>& key, byte& rcon)
{
    key[0u] ^= s_sbox[key[29u]] ^ rcon;
    key[1u] ^= s_sbox[key[30u]];
    key[2u] ^= s_sbox[key[31u]];
    key[3u] ^= s_sbox[key[28u]];
...
AES256_Base::expand_dec_key(Byte_Block<32> & key,
                            byte           & rcon) 
{
    for (size_t i = 31; i >= 20; --i)
        key[i] ^= key[i - 4];

    for (size_t i = 19; i >= 16; --i)
        key[i] ^= s_sbox[key[i - 4]];
...
void
AES256_Base::add_round_key(Byte_Block<16>& buf, Byte_Block<16> const& key)
{
    buf ^= key;
}
...
main()
{
    ...
    block = txt = "Short";
    std::cout << aes.decrypt(aes.encrypt(block)).as_string(true) << std::endl;
...
main()
{
    // Key is just all bytes set to 0

    unsigned char const kd[32] = {0x0};
    Byte_Block<32> key(kd);

    // Plaintext starts with 0x80, the rest is all 0

    unsigned char const td[16] = {0x80};
    Byte_Block<16> txt(td);

    std::cout << "Plaintext:  " << txt << std::endl;

    // Data of expected result of encryption

    unsigned char const rd[] = {0xdd, 0xc6, 0xbf, 0x79, 0x0c, 0x15, 0x76, 0x0d,
    ...
    std::cout << "Decryption: " << res << " -> "
              << (res == txt ? "Success" : "Failure") << std::endl;
