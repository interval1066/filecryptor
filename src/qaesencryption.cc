...
QByteArray
QAESEncryption::Crypt(QAESEncryption::Aes level, QAESEncryption::Mode mode, const QByteArray &rawText,
...
QByteArray
QAESEncryption::ExpandKey(QAESEncryption::Aes level, QAESEncryption::Mode mode, const QByteArray &key)
{
     return QAESEncryption(level, mode).expandKey(key);
}

QByteArray
QAESEncryption::RemovePadding(const QByteArray &rawText, QAESEncryption::Padding padding)
{
    if (rawText.isEmpty())
        return rawText;
...
QByteArray
QAESEncryption::getPadding(int currSize, int alignment)
{
...
QByteArray
QAESEncryption::expandKey(const QByteArray &key)
{
#ifdef USE_INTEL_AES_IF_AVAILABLE
  if (true){
      switch(m_level) {
...
void
QAESEncryption::addRoundKey(const quint8 round, const QByteArray &expKey)
{
...
QByteArray
QAESEncryption::byteXor(const QByteArray &a, const QByteArray &b)
{
...
QByteArray
QAESEncryption::cipher(const QByteArray &expKey, const QByteArray &in)
{
...
QByteArray
QAESEncryption::invCipher(const QByteArray &expKey, const QByteArray &in)
{
...
QByteArray
QAESEncryption::printArray(uchar* arr, int size)
{
    QByteArray print("");
...
QByteArray
QAESEncryption::encode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv)
{
...
QByteArray
QAESEncryption::decode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv)
{
    if (m_mode >= CBC && (iv.isEmpty() || iv.size() != m_blocklen))
       return QByteArray();
...
QByteArray
QAESEncryption::removePadding(const QByteArray &rawText)
{
