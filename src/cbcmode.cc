...
void
CBCMode::encrypt(std::istream& inStream, std::ostream& outStream)
{
    const int& blockSize = algorithm.getBlockSize();
...
void
CBCMode::decrypt(std::istream& inStream, std::ostream& outStream)
{
    const int& blockSize = algorithm.getBlockSize();
...
void
CBCMode::addBlock(uint8_t* a, uint8_t* b, const int& blockSize)
{
    for (int byte = 0; byte < blockSize; ++byte)
        a[byte] ^= b[byte];
}
