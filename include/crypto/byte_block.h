...
#if ! defined BYTE_BLOCK_H_
#define BYTE_BLOCK_H_
...
namespace crypto
{
    ...
    public:
        ...
        template<size_t M>
        Byte_Block(Byte_Block<M> const& other, size_t offset = 0) :
            m_init_len(N)
        {
    ...
    Byte_Block(unsigned char const* data, size_t length = N) :
        m_init_len(N)
    {
        if (length >= N)
            std::copy(data, data + N, m_data);
    ...
    // at the first zero byte. Bytes beyond the size of the object are
    // silently dropped and if there are less bytes padding is added.

    Byte_Block(char const* data, size_t length = 0) :
        m_init_len(N)
    {
		if (length == 0)
			length = strlen(data);
    ...
    // its data
    Byte_Block(std::string const& str, size_t offset = 0) :
        m_init_len(N)
    {
    ...
    {
        return ! std::memcmp(m_data, other.m_data, N);
    }
	...
	// Assignment operator from other Byte_Block object (of possibly
    // different size) or a std::string
    template<size_t M>
    Byte_Block&
    operator = (Byte_Block<M> & other)
    {
        if (other == *this)
            return *this;
	...
	template<size_t M>
	Byte_Block&
	set(Byte_Block<M> const & src, size_t offset = 0)
	{
		std::copy(src.m_data, src.m_data + std::min(N - offset, M), m_data + offset);
		return *this;
	}
	...
	// or assigned value had less bytes). How that's done depends on
    ...
    template<size_t N>
    std::ostream&
    operator << (std::ostream& out, Byte_Block<N> const& block)
    {
